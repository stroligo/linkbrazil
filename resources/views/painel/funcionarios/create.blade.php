        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('painel.funcionarios.store') }}" class="space-y-6">
                        @csrf
                                ...
                                <div class="form-field">
                                    ...
                                    <select id="mercado_id" name="mercado_id" class="form-select" required>
                                        ...
                                        @foreach($mercados as $mercado)
                                            <option value="{{ $mercado->id }}" {{ old('mercado_id') == $mercado->id ? 'selected' : '' }}>
                                                {{ $mercado->nome }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="form-section mt-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="form-field">
                                    ...
                                    <x-text-input id="password" name="password" type="password" class="form-input" required />
                                    <x-input-error class="form-error" :messages="$errors->get('password')" />
                                </div>

                                <div class="form-field">
                                    <x-input-label for="password_confirmation" :value="__('Confirmar Senha')" />
                                    <x-text-input id="password_confirmation" name="password_confirmation" type="password" class="form-input" required />
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="form-field">
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="status" value="1" class="form-checkbox" {{ old('status', true) ? 'checked' : '' }}>
